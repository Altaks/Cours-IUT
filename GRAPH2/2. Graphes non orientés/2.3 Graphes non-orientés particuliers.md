...
\node[color=orange!80,above,font=\large\bfseries] at (current bounding box.north) {$K_5$};
...
Un graphe est biparti s'il existe une partion de son ensemble de sommets en deux sous-ensembles $X$ et $Y$ telle que chaque arête ait une extrémité dans $\color{lime}X$ et l'autre dans $\color{yellow}Y$.
